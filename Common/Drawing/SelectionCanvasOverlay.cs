...
internal sealed class SelectionCanvasOverlay : IComposableOverlay
{
    ...
    public void Draw(SpriteBatch spriteBatch, OverlayContext context)
    {
        ...
        if (canvasActive)
        {
            ...
            var effectiveFill = (isDelimitationWand && settings.Mode == DelimitationWandMode.CanvasEdit)
                ...
                : canvasFill;
            StencilOverlayRenderer.DrawFill(spriteBatch, swp.Canvas.Tiles, screenBounds, effectiveFill);
        }
        ...
        if (canvasActive)
            StencilOverlayRenderer.DrawOutsideFill(spriteBatch, swp.Canvas.Tiles, screenBounds, outsideColor);
        ...
        if (selectionActive && settings.Mode != DelimitationWandMode.CanvasEdit)
            StencilOverlayRenderer.DrawFill(spriteBatch, swp.Selection.Tiles, screenBounds, tileSelColor);
    ...
    private static void DrawTransformIndicators(
        SpriteBatch sb,
        DelimitationWandPlayer swp,
        DelimitationWandSettings settings)
    {
        if (!TryResolveCentroidTile(swp, out var centroidTile))
            return;
        ...
        if (transformModeOn)
        {
            DrawTransformIcon(sb, centroidTile, usePivotIcon: false, DelimitationWandSettings.CanvasBorderColor);
            if (hasPivot)
                DrawTransformIcon(sb, pivotTile, usePivotIcon: true, DelimitationWandSettings.CanvasBorderColor);
            return;
        }
    ...
    private static bool TryResolveCentroidTile(DelimitationWandPlayer swp, out Vector2 centroidTile)
    {
